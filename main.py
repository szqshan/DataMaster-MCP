    ...
    try:
        if action == "list_sessions":
            ...
            if not session_id:
                result = {
                    "status": "error",
                    "message": "删除会话需要提供session_id参数"
                }
                return f"❌ 删除失败\n\n{json.dumps(result, indent=2, ensure_ascii=False)}"
            
            success, message = api_data_storage.delete_storage_session(session_id)
            ...
            if not session_id:
                result = {
                    "status": "error",
                    "message": "获取操作历史需要提供session_id参数"
                }
                return f"❌ 获取失败\n\n{json.dumps(result, indent=2, ensure_ascii=False)}"
            
            success, operations, message = api_data_storage.get_session_operations(session_id)
        ...
        logger.error(f"管理API存储失败: {e}")
        result = {
            "status": "error",
            "message": f"管理API存储失败: {str(e)}",
            "error_type": type(e).__name__
        }
        return f"❌ 操作失败\n\n{json.dumps(result, indent=2, ensure_ascii=False)}"
...
def analyze_api_storage_data(
    ...
    """
    分析临时数据库中的API数据，类似Excel的数据分析功能
    
    Args:
        session_id: 存储会话ID
        analysis_type: 分析类型 (preview|summary|filter|group|pivot|stats|export)
        columns: 要分析的列名列表
        filters: 过滤条件 {"column": "value", "age": ">18"}
        group_by: 分组列名列表
        aggregations: 聚合函数 {"column": "function"} (sum|avg|count|min|max)
        limit: 返回记录数限制
        sort_by: 排序列名
    ...
    try:
        if not session_id:
            result = {
                "status": "error",
                "message": "分析数据需要提供session_id参数"
            }
            return f"❌ 分析失败\n\n{json.dumps(result, indent=2, ensure_ascii=False)}"
        
        # 获取原始数据
        success, raw_data, message = api_data_storage.get_stored_data(
            session_id=session_id,
            format_type="json"
        )
        ...
        try:
            ...
            if isinstance(raw_data, list) and len(raw_data) > 0:
                if isinstance(raw_data[0], dict) and 'processed_data' in raw_data[0]:
                    # 提取processed_data
                    processed_list = []
                    for item in raw_data:
                        if item.get('processed_data'):
                            processed_data = json.loads(item['processed_data']) if isinstance(item['processed_data'], str) else item['processed_data']
                            if isinstance(processed_data, list):
                                processed_list.extend(processed_data)
                            else:
                                processed_list.append(processed_data)
                    df = pd.json_normalize(processed_list) if processed_list else pd.DataFrame()
                else:
        ...
        if analysis_type == "preview":
            # 数据预览
            preview_data = df.head(limit or 10)
            result = {
                "status": "success",
                "message": "数据预览成功",
                "data": {
                    ...
                    if column in df.columns:
                        if isinstance(condition, str) and any(op in condition for op in ['>', '<', '>=', '<=', '==', '!=']):
                            ...
                            try:
                                ...
                                filtered_df = filtered_df[filtered_df[column].astype(str).str.contains(str(condition), na=False)]
                        else:
                            # 精确匹配或包含
            ...
            result = {
                "status": "error",
                "message": f"不支持的分析类型: {analysis_type}",
                "supported_types": ["preview", "summary", "filter", "group", "stats"]
            }
            return f"❌ 分析类型错误\n\n{json.dumps(result, indent=2, ensure_ascii=False)}"
